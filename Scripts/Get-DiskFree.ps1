    ...
    PROCESS
    {
        foreach ($computer in $ComputerName)
        {
            try
            {
                ...
                {
                    $disks = Get-CimInstance -Query $wmiq -ErrorAction Stop
                }
                elseif ($Credential -eq [System.Management.Automation.PSCredential]::Empty)
                {
                    $disks = Get-CimInstance -Query $wmiq `
                ...
                else
                {
                    ...
                    $disks = Get-CimInstance -Query $wmiq `
                             -CimSession $cimSession -ErrorAction Stop
                }
